...
def save_test_result_to_firestore(user_id: str, answers: Dict[int, str], questions: List[Dict], threshold: int, start_time: float):
    ...
    try:
        ...
        if doc_idd in existing_data:
            if student_id in existing_data[doc_idd]:
  
                existing_data[doc_idd][student_id].append(taken_test)
            else:
 
                existing_data[doc_idd][student_id] = [taken_test]
        else:
